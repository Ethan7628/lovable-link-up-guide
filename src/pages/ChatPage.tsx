...
const ChatPage = () => {
  ...
  return <div className="min-h-screen bg-gradient-to-br from-purple-50 via-pink-50 to-rose-50 overflow-hidden">
      {/* Header */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 h-[calc(100vh-8rem)]">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-full">
              ...
              <CardContent className="p-0 flex-1 overflow-hidden">
                <ScrollArea className="h-full">
                  <div className="space-y-1 p-4">
                    {filteredChats.map(chat => <motion.div key={chat.id} whileHover={{
                    scale: 1.02
                  }} whileTap={{
                    scale: 0.98
                  }} className={`p-4 cursor-pointer transition-colors rounded-lg ${selectedChat?.id === chat.id ? 'bg-purple-50 border-r-4 border-purple-500' : 'hover:bg-gray-50'}`} onClick={() => setSelectedChat(chat)}>
                        <div className="flex items-center space-x-3">
                          <div className="relative flex-shrink-0">
                            ...
                            {chat.online && <div className="absolute bottom-0 right-0 h-3 w-3 bg-green-500 rounded-full border-2 border-white"></div>}
                          </div>
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center justify-between">
                              <p className="font-medium text-gray-900 truncate">{chat.name}</p>
                              <div className="flex items-center space-x-2 flex-shrink-0">
                                {chat.unread > 0 && <Badge className="bg-purple-600 text-white text-xs">
                                    {chat.unread}
                                  </Badge>}
          ...
          <div className="lg:col-span-2 h-full">
            {selectedChat ? <Card className="h-full bg-white/95 backdrop-blur-sm border-0 shadow-lg flex flex-col">
                {/* Chat Header */}
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                      <div className="relative">
                        ...
                        {selectedChat.online && <div className="absolute bottom-0 right-0 h-3 w-3 bg-green-500 rounded-full border-2 border-white"></div>}
                      </div>
                      <div>
                ...
                <CardContent className="flex-1 overflow-hidden p-0">
                  <ScrollArea className="h-full">
                    ...
                    duration: 0.3
                          ...
                          <div className={`max-w-xs lg:max-w-md px-4 py-2 rounded-lg break-words ${msg.isOwn ? 'bg-gradient-to-r from-purple-600 to-pink-600 text-white' : 'bg-gray-100 text-gray-900'}`}>
                ...
                <div className="border-t border-gray-200 p-4 flex-shrink-0">
                  <div className="flex items-center space-x-2">
                    <Input placeholder="Type a message..." value={message} onChange={e => setMessage(e.target.value)} onKeyPress={handleKeyPress} className="flex-1" />
                    <Button onClick={sendMessage} className="bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700" size="sm">
                      <Send className="h-4 w-4" />
                    </Button>
                  </div>
                </div>
              </Card> : <Card className="h-full bg-white/95 backdrop-blur-sm border-0 shadow-lg flex items-center justify-center">
